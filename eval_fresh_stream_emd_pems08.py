...
def main(args):
    ...
    rows = [
        make_acc({'method': 'DiffSTG Teacher (100-step)', 'NFE': 100, 'num_samples': 32, 'teacher_reference_samples': 32, 'checkpoint': args.teacher_checkpoint}),
        make_acc({'method': 'DiffSTG Teacher (40-step)', 'NFE': 40, 'num_samples': 32, 'teacher_reference_samples': 32, 'checkpoint': args.teacher_checkpoint}),
    ]
    ...
    with torch.no_grad():
        for batch_i, batch in enumerate(loader):
            ...
            if device.type == 'cuda':
                torch.cuda.synchronize(device)
            rows[1]['inference_time_sec'] += timer() - start
            rows[1]['num_eval_batches'] += 1
            update_gt(rows[1], teacher40, target)
            update_dist(rows[1], teacher40, teacher100)

            for row, (_, model, _, ns) in zip(rows[2:], students):
                if device.type == 'cuda':
                    torch.cuda.synchronize(device)
                start = timer()
    ...
    json_path = os.path.join(args.output_dir, 'fresh_stream_emd_comparison.json')
    csv_path = os.path.join(args.output_dir, 'fresh_stream_emd_comparison.csv')
